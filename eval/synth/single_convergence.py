...
def single_convergence_plot(f1, f2, R, t, max_iters=5000):
    sigma = 1.0
    ...
    for i in np.arange(2, 9, 2):
        val = 10.0 ** (-i)
        plt.plot(np.arange(0, max_iters + 1), [0, *percentile_under(rel_change, val)], label='$10^{-' + str(i) + '}$')
...
def single_eth3d_convergence_plot(max_iters=5000):
    eng = matlab.engine.start_matlab()
    s = eng.genpath('matlab_utils')
    eng.addpath(s, nargout=0)
    ...
    save_string, subsets, _, _ = get_subset_string('eth3d_multiview')
    ...
    for sample in tqdm(samples):
        ...
        if s != (1, max_iters + 1):
            # continue
            print("Warning: ", s)
            iters_list.append(max_iters + 1)
            errs_list.append(np.array([[3 - 1**(-i) for i in range(max_iters + 1)]]))
        else:
            iters_list.append(iters)
            errs_list.append(errs)


    errs = np.array(errs_list, dtype=float)[:, 0, :]
    ...
    for i in np.arange(2, 9, 2):
        val = 10.0 ** (-i)
        plt.plot(np.arange(0, max_iters), percentile_under(both, val), label='$10^{-' + str(i) + '}$')
...
if __name__ == '__main__':
    ...
    # single_convergence_plot(*set_scene(600, 600, theta=0, y=300), max_iters=20)
    # plt.xticks([0, 5, 10, 15, 20])
    # plt.savefig('figs/synth/single_convergence_noncoplanar.pdf')
    # plt.figure()
    #
    # plt.figure(figsize=(0.8 * 6, 0.6 * 4))
    # single_convergence_plot(*set_scene(600, 600, theta=0, y=0), max_iters=200)
    # plt.xticks([0, 50, 100, 150, 200])
    # plt.savefig('figs/synth/single_convergence_coplanar.pdf')
    # plt.figure()

    plt.figure(figsize=(0.8 * 6, 0.6 * 4))
    single_eth3d_convergence_plot(max_iters=500)
    plt.savefig('figs/synth/single_convergence_eth3d.pdf')
