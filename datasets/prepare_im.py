...
def cam_pair_generator(matcher, dataset_path, img_path, cameras, images, pts, min_overlap=10, max_pairs=100, seed=42, verbose=0):
    np.random.seed(seed)
    random.seed(seed)
    output = 0
...
def run_im(args):
    ...
    if args.num_workers == 1:
        for subset in dir_list:
            prepare_single(args.num_samples, basename, dataset_path, matcher, subset)
    else:
        ...
        pool.starmap(prepare_single, zip(num_samples, basenames, dataset_paths, matchers, dir_list))




def prepare_single(num_samples, basename, dataset_path, matcher, subset):
    subset_path = os.path.join(dataset_path, subset)
    if basename == 'phototourism':
